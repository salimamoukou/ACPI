...
def test_prediction_pi_qrf():
    ...
    y_train, y_test = np.asarray(y_train).astype(np.double).reshape(-1), np.asarray(y_test).astype(np.double).reshape(-1)
...
def test_prediction_pi_all():
    ...
    y_lower_lcprfg, y_upper_lcprfg = acp.predict_pi(x_test, method='lcp-rf-group')

    coverage_qrf = compute_coverage(y_test, y_lower_qrf, y_upper_qrf )
    coverage_lcprf = compute_coverage(y_test, y_lower_lcprf, y_upper_lcprf )
    coverage_lcprfgroup = compute_coverage(y_test, y_lower_lcprfg, y_upper_lcprfg)
